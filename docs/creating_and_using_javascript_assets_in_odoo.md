...
/web/content/499-0f03c7b/web.assets_common.js--ENTER--
/web/content/497-0f03c7b/web.assets_common.css--ENTER--
/web/content/500-ae10c27/web.assets_backend.js--ENTER--
/web/content/498-ae10c27/web.assets_backend.css--ENTER--
/web/webclient/qweb/bf598b302fae6d64cae29a50530a9ad7b19bb621?mods=base,web,...

The first two lines are the output of the assets\_common bundle. Bundles
...
assets\_backend bundle, and the last request is the call to fetch all
...
asset\_frontend bundle, and you\'re right. The asset\_common bundle
...
named \"web.assets\_backend\" and add a script tag inside of this
inherit with an xpath expression. Odoo knows how to build an asset
bundle by first rendering the \"web.assets\_backend\" template - after
...
**Tip**: don\'t forget to add this XML file to your \"\_\_manifest.py\_\_\" file!
...
\"/static/src/js\" named \"hello\_world.js\". Add this code for now and
...
are defined by a name (\"hello\_world.main\" here), and a function. That
...
dev=all flag or if Odoo is accessed in debug=assets mode.\
Refresh your browser. You\'ll now see a message in your web browser its
...
registers it in the \"action\_registry\" object. This is a very common
...
Once this is done, \"hello\_world.action\" is a valid action name from
...
client: the first part (odoo.\_\_DEBUG\_\_.services\[\"web.web\_client\"\])
gives you a reference to the instance of the WebClient widget that is currently
displayed in the UI. Then, we call the do\_action method to ask the web
...
We also have to update our \"\_\_manifest.py\_\_\" file so that Odoo
...
Let\'s start by creating a new XML file. Name it \"hello\_world.xml\"
...
your \"\_\_manifest.py\_\_\" file and add in a new key \"qweb\" which
loads our XML file:
...
\"hello\_world.js\" file again and modify \"OurAction\" so that it loads
the template:
...
Create a new file named \"hello\_world.css\" under \"static/src/css\"
